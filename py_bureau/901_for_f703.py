...
# load train
# =============================================================================
files = ('../feature_bureau/train_' + imp.head(HEAD).feature + '.f').tolist()

X_train = pd.concat([
                pd.read_feather(f) for f in tqdm(files, mininterval=60)
               ], axis=1)
y_train = utils.read_pickles('../data/bureau_label').TARGET
...
sub_train = utils.read_pickles('../data/bureau_train', ['SK_ID_CURR', 'SK_ID_BUREAU']).set_index('SK_ID_CURR')
...
# load test
# =============================================================================
files = ('../feature_bureau/test_' + imp.head(HEAD).feature + '.f').tolist()

X_test = pd.concat([
                pd.read_feather(f) for f in tqdm(files, mininterval=60)
               ], axis=1)

sub_test = utils.read_pickles('../data/bureau_test', ['SK_ID_CURR', 'SK_ID_BUREAU']).set_index('SK_ID_CURR')
...
sub_train.to_feather('../data/bureau_train_imputation_f703.f')
sub_test.to_feather('../data/bureau_test_imputation_f703.f')
