...
def multi_gr2(k):
    ...
    for v in col_num:
        ...
        base[f'{name}_std']  = gr1.std()
        base[f'{name}_nunique'] = gr1.apply(nunique)
    base.to_pickle(f'../data/tmp_301-2-{PREF}_{k}.p')
...
# merge
# =============================================================================
df = pd.concat([ pd.read_pickle(f) for f in sorted(glob(f'../data/tmp_301-2-{PREF}*.p'))], axis=1)
...
utils.to_pickles(train, '../data/301-2_train', utils.SPLIT_SIZE)
utils.to_pickles(test,  '../data/301-2_test',  utils.SPLIT_SIZE)

os.system('rm ../data/tmp_301-2-*.p')
